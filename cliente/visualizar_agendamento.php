...
try {
    ...
    // 2. Buscar TODOS os detalhes do agendamento, endereço e prestador
    $stmt = $pdo->prepare(
        "SELECT a.id, s.titulo AS titulo_servico, s.descricao AS descricao_servico, 
         ...
         WHERE a.id = ? AND a.Cliente_id = ?" // Garante que só verá o próprio agendamento
    );
    ...
    error_log("Erro em visualizar_agendamento.php: " . $e->getMessage());
    header("Location: meus_agendamentos.php");
    exit();
}
...
<main class="d-flex">
    ...
    <div class="container-fluid p-4">
        <h1 class="mb-4">Detalhes do Agendamento #<?= htmlspecialchars($agendamento_detalhes['id']) ?></h1>
        <hr>
