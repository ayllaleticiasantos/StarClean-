...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($erros)) {
        ...
        try {
            // Busca as coordenadas novamente ao editar
            $coordenadas = obterCoordenadasComNominatim($logradouro, $numero, $cidade, $uf);
            $latitude = $coordenadas['latitude'] ?? null;
            $longitude = $coordenadas['longitude'] ?? null;

            $stmt = $pdo->prepare(
                // Adiciona latitude e longitude ao UPDATE
                "UPDATE Endereco SET cep = ?, logradouro = ?, numero = ?, complemento = ?, bairro = ?, cidade = ?, uf = ?, latitude = ?, longitude = ?
                 WHERE id = ? AND Cliente_id = ?"
            );
            // Adiciona as novas variáveis ao execute
            $stmt->execute([$cep, $logradouro, $numero, $complemento, $bairro, $cidade, $uf, $latitude, $longitude, $id_endereco, $id_cliente]);
